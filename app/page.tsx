...
export default async function Home() {
  // Načtení dat z databáze pro zobrazení obsazenosti
  let rows: any[] = [];
  try {
  ...
  // Funkce pro vytvoření rezervace (Server Action)
  async function createReservation(formData: FormData) {
    ...
    try {
      await sql`
        ...
        VALUES (${mistnost as string}, ${jmeno as string}, ${datum as string})
      `;
      revalidatePath('/'); // Obnoví stránku, aby se zobrazila nová rezervace
    } catch (e) {
      ...
      <div className="container" id="home">
        ...
        <header className="card hero" style={{ textAlign: 'center', padding: '60px 20px' }}>
          <h1 style={{ fontSize: '3.5rem', marginBottom: '20px' }}>
            Pracujte lépe v <span style={{ color: '#2563eb' }}>HubSpace</span>
          </h1>
          ...
          <div className="card" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <details style={{ padding: '15px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', cursor: 'pointer' }}>
                <summary style={{ fontWeight: 'bold' }}>Jak funguje přístup do prostor?</summary>
                <p style={{ marginTop: '10px', color: '#64748b' }}>Po potvrzení rezervace obdržíte do e-mailu unikátní kód ke dveřím.</p>
              </details>
              <details style={{ padding: '15px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', cursor: 'pointer' }}>
                <summary style={{ fontWeight: 'bold' }}>Jaká je rychlost internetu?</summary>
                <p style={{ marginTop: '10px', color: '#64748b' }}>V celém objektu máme symetrickou optiku 1 Gbps přes WiFi 6.</p>
              </details>
            </div>
          </div>
        </section>
